...
def art(title, slug, category_slug, excerpt, body_markdown, author_name, author_type, author_slug, date_published, source_name, source_url, source_type, image_url=None, image_alt="", image_caption="", image_credit="", image_license="", license_url=None, tags=[], notes=None):
    missing_fields = []
    images = []
    ...
    return {
        ...
        "source": {
            ...
            "sourceLanguage": "vi",
            "sourceReliabilityNote": f"Nguồn tin {source_type} uy tín từ {source_name}, xác minh thông tin gốc."
        },
        "images": images,
        "verification": {
            "factsChecked": True,
...
Nghiên cứu hành vi tiêu dùng bất động sản của nhóm khách hàng tuổi từ 22 đến 35 cho thấy định nghĩa về chỗ ở đã có sự thay đổi sâu sắc. Khách thuê trẻ không chỉ tìm kiếm một chỗ ngủ mà hướng tới một hệ sinh thái sống trọn vẹn (All-in-one).
...
3. **Căn hộ Full nội thất:** Ưu tiên thuê căn hộ đã trang bị đầy đủ thiết bị gia dụng để sẵn sàng dọn vào ở ngay, giảm chi phí đầu tư ban đầu.
...
categories_data = [
    ...
    {"slug": "dau-tu-dong-tien", "candidateCount": len(art_dau_tu), "articles": art_dau_tu},
